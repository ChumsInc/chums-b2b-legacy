...
const AccountUserTable = ({users, currentUser, onSelectUser}: {
    users: CustomerUser[];
    currentUser?: number;
    onSelectUser: (arg: CustomerUser) => void;
}) => {
    ...
    return (
        <div>
                    ...
                    .map(user => (
                        <tr key={`${user.id}:${user.shipToCode?.length}`} onClick={() => onSelectUser(user)}
                            ...
                            <td>
                                ...
                                {!user.shipToCode?.length && (
                                    <span title="Complete Account"><BusinessIcon fontSize="small"/></span>)}
                            </td>
                        </tr>
                    ))
                }
            ...
            <TablePagination count={users.length} onPageChange={(ev, page) => setPage(page)} page={page}
                             rowsPerPage={10}/>
        </div>
    )
}
