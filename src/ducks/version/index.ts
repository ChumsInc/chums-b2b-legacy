...
export const loadingSelector = (state:RootState):boolean => state.version.loading;
export const shouldCheckVersion = (state:RootState):boolean => !loadingSelector(state)
    && state.version.versionNo !== ''
    && (new Date().valueOf() - state.version.lastChecked > minCheckInterval);
export const changedSelector = (state:RootState): boolean => state.version.changed && state.version.versionNo !== state.version.ignored;
export const versionSelector = (state:RootState):string => state.version.versionNo;


const versionReducer = (state:VersionState = {...defaultState}, action:VersionAction):VersionState => {
