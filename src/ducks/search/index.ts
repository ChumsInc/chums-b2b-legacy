...
export const setSearchTerm = (term:string) => ({type: SET_SEARCH, term});
export const showSearch = (show:boolean) => ({type: SHOW_SEARCH, show});

export const getSearchResults = (term:string) => (dispatch:AppDispatch, getState: () => RootState) => {
    if (!term) {
