...
const OrderHeaderShipTo = () => {
    ...
    const changeHandler = (shipToCode:string) => {
    ...
    return (
        <>
            <FormGroup colWidth={8} label="Ship To">
                <ShipToSelect value={orderHeader.ShipToCode || ''} defaultName="Default Address" inputProps={{disabled: !isCart}}
                              onChange={changeHandler}/>
            </FormGroup>
            <FormGroup colWidth={8} label="Address">
                <DeliveryAddress address={orderHeader} className="form-control form-control-sm"/>
            </FormGroup>
        </>
    )
}
